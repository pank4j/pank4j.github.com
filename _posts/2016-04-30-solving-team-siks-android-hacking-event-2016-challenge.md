---
comments: true
date: 2016-04-30 19:38:07+00:00
layout: post
...
Team SIK organized a [CTF based hacking event](https://team-sik.org/ahe16-overview/) recently, which involved reverse engineering four android apps of varying difficulty levels, ranging from trivial to hard. Each app included a secret password hidden inside, which was to be found in order to complete that level. The apps are available at [https://goo.gl/vMj6b6](https://goo.gl/vMj6b6).
...
Solving the trivial challenge is as simple as running the app in Genymotion and entering an incorrect password. Run ``logcat`` to watch the logs as you enter a password. The app prints out the password in the logs when verifying for password.
{% highlight bash %}
